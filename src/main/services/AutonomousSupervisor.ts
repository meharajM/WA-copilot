...
export class AutonomousSupervisor extends EventEmitter {
  ...
  onDeliveryUpdate(update: { providerMessageId: string; status: string; timestamp: number; channel?: string }): void {
    ...
    if (typeof update?.providerMessageId !== 'string' || !update.providerMessageId.trim() || !Number.isFinite(update.timestamp) || update.timestamp <= 0 || !['sent', 'delivered', 'read', 'failed'].includes(update.status) || (update.channel !== undefined && !channels.has(update.channel))) {
