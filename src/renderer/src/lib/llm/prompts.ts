...
IMPORTANT: If the user has attached a file to this message, READ it immediately using convert_to_markdown with the exact uri shown in the attachment block. Attached files do NOT require a workspace.`}
  ...
  const toolsDescription =
    tools
      ?.map((tool, idx) => {
        ...
        // The "Connected MCP Servers" section below handles the grouping.
...
DO NOT stop after just navigating - complete the entire workflow!`;
  ...
  const isWaEnabled = whatsappState.whatsappEnabled || whatsappState.businessBotMode;
  let whatsappNote = "";
...
IMPORTANT: If the user has attached a file, READ it immediately using convert_to_markdown with the exact uri shown in the attachment block. Attached files do NOT require a workspace to be read.`}
...
2. **Immediate Action**: 
   - **Audio/Voice Notes (.ogg, .mp3, etc.)**: Always call \`convert_to_markdown\` immediately to get the transcript. Treat the transcript as the user's spoken command.
   - **Spreadsheets (.xlsx, .xls, .csv, .ods, .tsv)**: Always call \`convert_to_markdown\` immediately to read the tabular data as a Markdown table. Then reason over the data.
   - **Documents (.pdf, .docx, .pptx, etc.)**: Always call \`convert_to_markdown\` immediately to read the content.
...
3. **WhatsApp Audio (.ogg)**: WhatsApp voice notes are always \`.ogg\` files. You MUST use \`convert_to_markdown\` to "hear" them. DO NOT reply until you have the transcript.
...
1. **Attachment Fast Path**: If an [ATTACHED FILES] block provides a \`convert_to_markdown(uri="...")\` call, copy that exact \`uri\` value. It may be a \`file://\` URI and does not require workspace verification.
2. **Verify Other Files First**: Before using a non-attached file in a tool, verify its existence and path using 'search_files' or 'list_directory'.
3. **Filesystem Tool Paths**: For fs_* tools, use workspace-relative paths or raw absolute paths (e.g., '/Users/username/Documents/file.txt'). Do not add a \`file://\` prefix to fs_* tool paths.
4. **No Assumptions**: Do NOT assume a file is in the project root. Search for it if the user provides a filename only.



# AVAILABLE TOOLS
${toolsDescription}${serverContext}${browserCapabilityNote}${whatsappNote}${ragNote}
