...
- `agentd` owns the loopback HTTP listener, pairing/session state and durable product database. The native host validates the private runtime descriptor and reuses a live daemon; it starts a packaged child only when no valid daemon is available. While the companion is running, its supervisor reaps and restarts a child that it started after a crash, while refusing to start a second writer when another valid descriptor exists. Closing the browser or companion does not stop a running `agentd`; explicit agent controls own processing state. Automatic service installation, recovery after the companion itself exits, and OS user-service registration are not yet release-complete behaviors.
