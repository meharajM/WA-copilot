...
- Browser sends fail closed when the selected transport or credentials are unavailable. The migrated Baileys path supports explicit text sends, approved drafts, and Autonomous Bot Mode through the same idempotent text-only outbox; `agentd` records a durable pending/sent/failed entry and suppresses duplicate provider calls for either Baileys or Cloud. Failed attempts can be retried through the authenticated browser route; an operator can quarantine or cancel a failed attempt, and those dispositions block accidental re-send. Pending provider work cannot be cancelled or quarantined and returns an explicit conflict. WhatsApp Web automation, media delivery, escalation/courtesy/follow-up parity and full continuity remain unavailable.
