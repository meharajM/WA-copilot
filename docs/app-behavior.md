...
- Cutover state, token consumption, idempotency, and backup references survive agentd restart in SQLite; status never returns token or credential values. Rollback verifies the backup's SHA-256 in constant time and keeps `needs-recovery` plus the migration hold active on tamper or restore failure.
