...
- Browser Autonomy delivery history combines bounded sent/failed Email draft events with WhatsApp outbox events through authenticated agentd. Email uses a durable local `email:<draft-id>` delivery ID because the current SMTP/Gmail sender path does not persist a provider receipt; richer provider delivery-event parity remains open.
...
- Native diagnostics exposes presence/write-only operations for every public agentd credential (LLM, Email app-password, Gmail OAuth client ID, and WhatsApp Cloud keys). Gmail OAuth refresh tokens remain internal to the agentd OAuth flow and cannot be set, read, or deleted through the native bridge. Reauthentication still requires the owner to enter supported values through the browser's authenticated settings flow; the native screen only inventories what must be re-entered.
