...
- Windows credential continuity remains a release gate until the native no-reparse descriptor guarantee and a packaged Credential Manager runtime test are both evidenced. Credential values, OAuth refresh tokens, and decrypted Electron `safeStorage` data must never appear in logs, temporary files, browser responses, or renderer state.
...
- Windows no-reparse-safe native descriptor support, credential/keychain handoff, Electron retirement, and release smoke remain open gates. Native bridge/UI wiring is now present for this bounded scope, but it does not claim full continuity migration or Windows release readiness.
