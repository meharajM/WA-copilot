...
- An optional browser-extension bridge is available only when `AICA_EXTENSION_BRIDGE_TOKEN` is configured. In the browser-first runtime it is owned by `agentd`; in the Electron transition runtime the legacy bridge remains available. It binds to `127.0.0.1`, accepts bounded authenticated status and inbound text-message JSON, and forwards messages to the daemon's durable WhatsApp ingress. It has no outbound send, arbitrary browser evaluation, shell, or filesystem operation. Official webhooks still require an explicitly configured, authenticated reachable relay for production deployment; localhost listeners are for controlled development/pilot use.
