...
- WhatsApp Web uses a dedicated persistent Playwright profile, manual QR/session restoration, bounded incoming DOM monitoring, screenshots, and human takeover. The browser-first extension now has bounded explicit owner-chat text/media command/ack paths with active-chat, composer/file-input and send-control checks; autonomous Web dispatch and provider delivery semantics remain disabled until live validation.
...
- An optional browser-extension bridge is available only when `AICA_EXTENSION_BRIDGE_TOKEN` is configured. In the browser-first runtime it is owned by `agentd`; in the Electron transition runtime the legacy bridge remains available. It binds to `127.0.0.1`, accepts bounded authenticated status/inbound text JSON, and carries one owner-chat outbound text/media command/ack only after the visible chat, composer/file input and send control are feature-detected. It has no navigation, arbitrary browser evaluation, shell, or filesystem operation outside the selected media command. Official webhooks still require an explicitly configured, authenticated reachable relay for production deployment; localhost listeners are for controlled development/pilot use.
