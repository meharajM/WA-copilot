...
- An optional browser-extension bridge is available only when `AICA_EXTENSION_BRIDGE_TOKEN` is configured. It binds to `127.0.0.1`, accepts bounded authenticated status and inbound text-message JSON, and forwards messages to the main supervisor. It has no outbound send, arbitrary browser evaluation, shell, or filesystem operation. Official webhooks still require an explicitly configured, authenticated reachable relay for production deployment; localhost listeners are for controlled development/pilot use.
