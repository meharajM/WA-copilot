...
- WhatsApp Web uses a dedicated persistent Playwright profile, manual QR/session restoration, bounded incoming DOM monitoring, screenshots, and human takeover. The browser-first extension now has a bounded explicit owner-chat text command/ack path with active-chat/composer checks; autonomous Web dispatch, media and provider delivery semantics remain disabled until live validation.
...
- An optional browser-extension bridge is available only when `AICA_EXTENSION_BRIDGE_TOKEN` is configured. In the browser-first runtime it is owned by `agentd`; in the Electron transition runtime the legacy bridge remains available. It binds to `127.0.0.1`, accepts bounded authenticated status/inbound text JSON, and carries one owner-chat outbound text command/ack only after the visible chat and composer are feature-detected. It has no media, navigation, arbitrary browser evaluation, shell, or filesystem operation. Official webhooks still require an explicitly configured, authenticated reachable relay for production deployment; localhost listeners are for controlled development/pilot use.
