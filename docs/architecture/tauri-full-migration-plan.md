...
- 2026-09-18: Removed the unreachable Tauri product chat preview and made the browser WhatsApp connection dialog fail closed with an explicit Cloud API/native-transport migration message instead of presenting a QR flow that can only call Electron. Electron QR/session behavior remains unchanged.
