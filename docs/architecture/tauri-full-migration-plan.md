...
- 2026-09-18: Closed the browser chat streaming/cancellation gap. Authenticated browser generations now request bounded provider SSE, forward validated assistant deltas, persist the completed assistant message only after a clean done event, and expose a CSRF-protected cancellation route that aborts provider work and permits safe retry. Browser cancellation no longer renders a spurious error bubble; Electron behavior remains unchanged.
