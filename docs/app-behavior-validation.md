...
The 2026-09-21 browser-storage hardening removed the last generic `electron.store` renderer-local fallback. Electron continues delegating to its native store; Edge/Chrome calls now return defaults or fail closed with an explicit agentd-owned-storage error, while product settings remain persisted through authenticated agentd routes. Focused boundary tests, 346 unit tests, renderer typecheck, and browser pairing/reload E2E passed after this change.
