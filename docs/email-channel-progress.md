...
When the browser Email channel is enabled and Auto-Reply is explicitly on, the browser claims this bounded queue, creates or reuses the deterministic agentd-backed email session, appends the normalized message once, runs authenticated generation, and applies the existing confidence/draft policy before acknowledging the event. With Auto-Reply off, daemon-ingested events remain durable and the UI reports that they are stored but gated; they are not silently routed into an agent session. Draft Mode, sensitive-topic handling, and approved delivery gates remain enforced. Approved drafts with optional bounded operator-selected attachments can use the separately gated daemon SMTP/Gmail route; successful or failed browser sends also emit the same redacted system delivery notification used by Electron. Provider delivery receipts remain unavailable, as do browser-side credential reads.
