...
Official webhooks require public HTTPS. The repository now includes one minimal Node HTTPS relay with SQLite (`relay/public-relay.cjs`) for a persistent single-host deployment before the real Cloud API pilot. The current localhost listeners are test adapters only; DNS/TLS, backup/restore and live provider callback evidence remain deployment gates. The relay is a required dependency of phase 2, not an optional post-pilot enhancement.
...
Current migration status (2026-09-22): the backend-first draft gate is closed for the bounded local WhatsApp text path, and authenticated browser sends now include bounded outbound image/video/audio/document media over Baileys or Cloud. Browser `businessBotMode` now uses authenticated agentd state and the provider-backed idempotent outbox (`draft -> approved -> send`); the browser Autonomy panel exposes the same mode and response-permission controls. Deterministic courtesy/admin-escalation outbox records, resolution logging and daemon-owned inactivity follow-up are also migrated for text/caption events. The local Baileys path now downloads supported inbound media within an 8 MiB cap, stores it privately, and exposes authenticated on-demand bytes; only small images are forwarded into model requests. WhatsApp Web now has a bounded authenticated owner-chat text/media command/ack bridge in the browser-first path, but live DOM/provider/send/media validation, autonomous Web dispatch, Windows release and full data-parity gates remain open.
...
Current implementation slice: continue migrating `App.tsx` workflows to authenticated `agentd` routes in vertical slices. Browser Email configuration/credential continuity and the bounded app-password IMAP/SMTP MIME path now use `agentd`; Gmail OAuth/API inbound metadata plus scanned small-image hydration are also browser-owned. IMAP/Gmail media bytes are private, integrity-checked and only small scan-compatible images may enter generation; larger or unsupported files remain operator-only. Richer tool/RAG policy, delivery events and unsupported provider transports remain gated until their adapters are complete. Tauri stays limited to native capabilities. Implement and test the selected public relay before live Cloud API ingress. Capacity measurements and terminal conversation outcomes are explicit acceptance gates; no checkbox is closed solely by this architecture decision.
