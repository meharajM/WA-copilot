...
Use the [`SHA256SUMS.txt`](downloads/SHA256SUMS.txt) file beside the downloads to verify the files before installing. In PowerShell, run `Get-FileHash '.\AICA Native Host_1.0.0_x64-setup.exe' -Algorithm SHA256` (or the quoted MSI path) and compare the result with the manifest. The public packaging workflow is [`Tauri Windows package`](https://github.com/meharajM/WA-copilot/actions/workflows/tauri-windows-package.yml); each run also uploads the unsigned NSIS/MSI pair as a commit-specific artifact.
...
The browser bundle was built locally with `npm run build:tauri:web`. Local verification passed: typecheck, lint with existing warnings, 349 Vitest unit tests, the dedicated speech-model cache suite, focused browser-extension DOM/bridge tests, 196 integration tests with 8 skips, Electron and browser E2E, Tauri web build, 20 Rust tests, and diff checks. The current local release gate is green; current-head Pages, Windows package, and full Windows workflows also pass (runs `35647398682`, `35647405087`, and `35647404916`). The Windows artifacts are unsigned evaluation outputs; signing and real-profile upgrade continuity remain release gates.
