...
- Lifecycle QA is browser-first: launch the packaged companion from its app icon and verify that Edge/Chrome opens the loopback workspace while the native diagnostics window remains hidden. Close and reopen the browser tab; the daemon must remain healthy and durable work must continue. Reopen diagnostics from the tray, verify **Keep running in background** defaults on, toggle it off/on, and exercise **Start background agent** and **Stop background agent** with the explicit confirmation. A stopped agent must stay stopped until an owner starts it again; a browser tab close must never be treated as an implicit stop request.
