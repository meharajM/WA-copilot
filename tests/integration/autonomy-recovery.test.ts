...
describe('autonomy recovery', () => {
  ...
  it('reconciles Cloud delivery failures into the inbound record', async () => {
    ...
    activeSupervisor.onDeliveryUpdate({ providerMessageId: 'wamid.failed-1', status: 'read', timestamp: Date.now(), channel: 'whatsapp' })
    expect(db.prepare('SELECT status FROM outbound_sends WHERE inbound_id = ?').get(inboundId)).toMatchObject({ status: 'delivered' })
    expect(db.prepare('SELECT status FROM delivery_events WHERE provider_message_id = ? ORDER BY id DESC LIMIT 1').get('wamid.failed-1')).toMatchObject({ status: 'read' })
