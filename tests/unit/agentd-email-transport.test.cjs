...
test('agentd sends an approved browser email draft through daemon transport', async () => {
  ...
  assert.match(result.body.draft.providerMessageId, /^smtp:<aica-draft_transport_1-[a-f0-9]+@localhost>$/)
  assert.equal(sent[0].messageId, result.body.draft.providerMessageId.slice('smtp:'.length))
...
test('agentd preserves approved browser attachments for daemon delivery', async () => {
  ...
  assert.equal(result.status, 200)
  assert.match(result.body.draft.providerMessageId, /^smtp:<aica-draft_transport_attachment_1-[a-f0-9]+@localhost>$/)
